...
#read image  
  img = NULL
  tryCatch(expr = {
    img <- readPNG(paste(kegg.dir, "/", pathway.name, ".png", sep = ""))
  }, error = function (err){
    warning(sprintf('reading %s png failed!',paste(kegg.dir, "/", pathway.name, ".png", sep = "")))
...
if(!is.null(cols.ts.gene) & nc.gene>=np){
    ...
    if(same.layer!=T){
      result=render.kegg.node(plot.data.gene, cols.gene.plot, img, same.layer=same.layer, type="gene", cex=cex)
...
if(!is.null(cols.ts.cpd) & nc.cpd>=np){
  ...
  if(same.layer!=T){
      result=render.kegg.node(plot.data.cpd, cols.cpd.plot, img, same.layer=same.layer, type="compound", cex=cex)
	  if(is.null(result)) return(invisible(NULL))
  } else{
...
# na.col=colorpanel2(1, low=na.col, high=na.col)
 ...
 na.col.cpd=ucol.cpd %in% c(na.col, NA)
  ...
  if(new.signature) pathviewmod.stamp(x=17, y=20, on.kegg=T, cex = pv.pars$sign.cex)
  par(pv.pars$op)
  dev.off()
}
