...
#  fillcol=matrix(rep(fillcol,nplots), ncol=nplots)
  ...
  if(!is.null(cols.ts.gene) & sum(rect.idx)>0){
    fillcol[nidx.gene,1:nc.gene]=cols.ts.gene[cidx.gene,] #need to be added in pathviewmod.graph
...
#    ellipses(xloc.e, yloc.e, w.e, h.e/2, cols=ell.col)
 ...
 na.col.cpd=ucol.cpd %in% c(na.col, NA)
  ...
  if(new.signature) pathviewmod.stamp(position=sign.pos, graph.sizes=pv.pars$gsizes, on.kegg=F, cex = pv.pars$sign.cex)
