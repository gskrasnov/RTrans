...
if __name__ == '__main__':
    ...
    for FileName in sys.argv[2:]:
        ...
        F_is_present = "F" in cell_index_by_name
        ...
        if LR_is_present:
            sheet.write(0, ColN, 'LR test', bold)
            excel_Col_index_by_Col_name['LR'] = ColN
            ColN += 1

        if F_is_present:
            sheet.write(0, ColN, 'F test', bold)
            excel_Col_index_by_Col_name['F'] = ColN
            ColN += 1

        if LR_is_present or F_is_present:  sheet.write(0, ColN, 'GLM P', bold)
        else:   sheet.write(0, ColN, 'ET P', bold)

        excel_Col_index_by_Col_name['GLM P'] = ColN
        ColN += 1

        if LR_is_present or F_is_present:  sheet.write(0, ColN, 'GLM FDR', bold)
        else:   sheet.write(0, ColN, 'ET FDR', bold)
        ...
        for n in range(CPM_cells_count):
            sheet.write(0,CPM_start_ColN + n,C[12 + (LR_is_present or F_is_present) + Summary_is_present + n])
        ...
        for L in open(FileName,'r').readlines()[1:]:
            ...
            if F_is_present:
                write_number__mod(sheet,stringN,excel_Col_index_by_Col_name['F'],C[cell_index_by_name['F']],format2)
            ...
            for n in range(CPM_cells_count):
                sheet.write_number(stringN,CPM_start_ColN + n,float(C[13 + (LR_is_present or F_is_present) + Summary_is_present + n]))
